...
async def get_classroom_detail_admin(classroom_id: str, current_user: UserModel = Depends(get_current_admin)):
    data = await classroom_service.get_classroom_by_id(classroom_id)
    return BaseResponse(data=data)


@router.put("/{classroom_id}", response_model=BaseResponse)
async def update_classroom_admin(classroom_id: str, update_data: CreateClassroomSchema, current_user: UserModel = Depends(get_current_admin)):
...
async def delete_classroom_admin(classroom_id: str, current_user: UserModel = Depends(get_current_admin)):
